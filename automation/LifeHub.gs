...
function validateIntent(intent, now = new Date().toISOString()) {
    ...
    if (intent.destination && !EXTRA_LISTS.some(list => list.key === intent.destination && list.kind === intent.kind))
        return 'Choisissez une liste compatible avec cette demande.';
...
function classify(text, reference = new Date().toISOString()) {
    ...
    if (explicitList) {
        ...
        if (!content)
            return review('Précisez le contenu à ajouter à la liste.');
    ...
    if (/^(?:regarder|voir)\s+(?:le film|la serie|le documentaire)\b/.test(normalized))
        return { kind: 'note', destination: 'watchlist', title };
    ...
    if (/^(?:acheter\b|courses\b|commissions\b|(?:ajoute|ajouter)\b.*\b(?:courses|commissions)\b|il faut acheter\b)/.test(normalized)) {
        ...
        const stores = [['leroy', /\b(?:chez|a|au) leroy merlin\b/], ['norauto', /\b(?:chez|a) norauto\b/], ['pets', /\b(?:a l'|en |chez l')animalerie\b/], ['pharmacy', /\b(?:a la |en )pharmacie\b/], ['supermarket', /\b(?:en grande surface|au supermarche)\b/]];
        const matches = stores.filter(([, pattern]) => pattern.test(normalized));
        if (matches.length > 1)
            return review('Plusieurs magasins sont cités. Séparez les demandes par magasin.');
