...
import { classify, packNotes, unpackNotes, validateIntent } from '../voice/engine'
import type { Intent, IntentKind, VoiceTask } from '../voice/engine'
import './voiceInbox.css'

type View = 'inbox' | 'review' | 'task' | 'shopping' | 'note' | 'history'
...
const TABS: { id: View; title: string }[] = [{ id: 'inbox', title: 'Réception' }, { id: 'review', title: 'À préciser' }, { id: 'task', title: 'Tâches' }, { id: 'shopping', title: 'Commissions' }, { id: 'note', title: 'Notes' }, { id: 'history', title: 'Historique' }]
...
export function VoiceInbox() {
  ...
  function capture() {
    if (!text.trim()) return
    void run(async () => {
      const draft = { id: crypto.randomUUID(), title: text.trim() }
  ...
  const counts: Record<View, number> = { inbox: drafts.length, review: 0, task: 0, shopping: workspace?.shopping.filter(t => t.status !== 'completed').length ?? 0, note: workspace?.notes.length ?? 0, history: 0 }
  tasks.forEach(task => counts[groupFor(task)]++)
  const displayed = (view === 'shopping' ? workspace?.shopping ?? [] : view === 'note' ? workspace?.notes ?? [] : tasks.filter(task => groupFor(task) === view)).slice().sort((a, b) => (b.updated || '').localeCompare(a.updated || ''))
